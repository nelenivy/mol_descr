...
void MoleculeManager::AppendCharges(std::vector<double>& charges)
{
	charges.reserve(charges.size() + m_non_marked_singular_points.first.size());

	for (auto iter = m_non_marked_singular_points.first.begin(); iter != m_non_marked_singular_points.first.end(); ++iter)
	{
		charges.push_back(iter->Property().ElectricPotential());
	}
}
void MoleculeManager::AppendLennardJones(std::vector<double>& lennard_jones)
{
	lennard_jones.reserve(lennard_jones.size() + m_non_marked_singular_points.first.size());
...
size_t MoleculeManager::GetPointsTypeNum()
{
	...
	return kSaddleType * (m_lennard_jones_threshes.size() + 1) * (m_charge_threshes.size() + 1);
}
...
int MoleculeManager::CalculateSingularPointsTypes(PropertiesSet& prop)
{
	...
	std::vector<TypeWithMax> type_with_max(3);
	enum {SIGN, LENNARD_JONES, CURVATURE};
...
void MoleculeManager::ReadAllSingularPoints()
{
